...
abstract class Base : View {
    ...
    internal var selectorPaint: Paint     ? = null            // selector paint
    internal var baseLayer: Bitmap           ? = null              // bitmap for the base layer
    internal var colorLayer: Bitmap           ? = null             // bitmap for the color layer
    internal var layersPaint: Paint           ? = null             // paint object for the layers
    internal var layersCanvas: Canvas          ? = null            // canvas object for the layers
    internal var padding: RectF             ? = null               // padding for the color window, so that the selector is kept inside
    ...
    fun onInitBase() {
        ...
        Log.d("TAGhhh", "onInitBase: $colorLayer")
    ...
    internal open fun drawBorder(canvas: Canvas) {
        ...
        layersPaint?.let { canvas.drawPath(clipPath, it) }
    }
    ...
    internal open fun drawSelector(canvas: Canvas) {
        ...
        selectorPaint?.let { canvas.drawCircle(selectorX, selectorY, selectorRadius + halfStrokeWidth, it) }
        selectorPaint?.let { canvas.drawCircle(selectorX, selectorY, selectorRadius - halfStrokeWidth, it) }

        // draw the original stoke
        selectorPaint?.color = selectorColor
        selectorPaint?.strokeWidth = selectorStrokeWidth
        selectorPaint?.let { canvas.drawCircle(selectorX, selectorY, selectorRadius, it) }
    }
        ...
        inline fun View.afterMeasured(crossinline function: View.() -> Unit) {
            viewTreeObserver.addOnGlobalLayoutListener(object :
                ...
                override fun onGlobalLayout() {
                    Log.d("TAGhhh", "onGlobalLayout: $measuredWidth-$measuredHeight")
