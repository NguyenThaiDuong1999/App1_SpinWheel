...
class SliderA : Slider {
    ...
    override fun onInit() {
        ...
        layersPaint?.let { layersCanvas?.drawRect(0f, 0f, width.toFloat(), height.toFloat(), it) }
        ...
        for (i in 0..rows) {
            for (j in 0..columns / 2) {
                ...
                layersPaint?.let { layersCanvas?.drawRect(left, top, right, bottom, it) }
            }
        }
        ...
        layersPaint?.let { layersCanvas?.drawPath(clipPath, it) }

        // draw zebra bitmap and cur base bitmap using SDT_IN mode
        layersCanvas = baseLayer?.let { Canvas(it) }
    ...
    override fun onRedraw() {
        ...
        // draw rainbow shader
        layersCanvas = colorLayer?.let { Canvas(it) }
        baseLayer?.let { layersCanvas?.drawBitmap(it, 0f, 0f, layersPaint) }
        layersPaint?.let { layersCanvas?.drawPath(clipPath, it) }
        invalidate()
    }
